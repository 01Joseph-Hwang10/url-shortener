...
> [!IMPORTANT]\
> 다른 모든 코드는 수정해도 무방하나, 테스트 파일에 있는 함수 중 `test_*`로 시작하는 함수들은 수정하시면 안됩니다. 이 함수들은 "테스트 코드"이고, 위에서 명세한 기능들이 제대로 구현되었는지를 확인하는 코드이기에, 해당 함수들을 수정하면 과제를 제대로 수행했는지 확인할 수 없게 됩니다.
